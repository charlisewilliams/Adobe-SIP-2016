...
y_local = int(SCREEN_HEIGHT/2)


ball_size = random.randint(10, 30)

# Ball 2
x_s = random.randint(-10, 10)
y_s = random.randint(-10, 10)
...
while not done:
    ...
    #Ball 2
    pygame.draw.circle(screen, ball_color, [x_location, y_local], ball_size)


    if x_location >= SCREEN_WIDTH - ball_size or x_location < ball_size:
        x_s = x_s * -1

    if y_local >= SCREEN_HEIGHT - ball_size or y_local < ball_size:
        y_s = y_s * -1
